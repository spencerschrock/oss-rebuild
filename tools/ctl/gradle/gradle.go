...
func RunPrintCoordinates(ctx context.Context, sourceRepo git.Repository, gradleCmdExecutor dockerlocal.CommandExecutor) (*GradleProject, error) {
	...
	err = gradleCmdExecutor.Execute(ctx, dockerlocal.CommandOptions{Dir: wt.Filesystem.Root(), Output: os.Stdout}, "./gradlew", "printCoordinates", "--init-script", "printCoordinates.gradle", "--no-daemon")
	if err != nil {
